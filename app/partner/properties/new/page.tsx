...
export default function NewPropertyPage() {
  ...
  const Input = ({ label, name, type = 'text', required = false, placeholder = '', ...rest }: any) => (
    <div>
      <label className="block text-sm font-medium text-slate-700 mb-1">{label} {required && <span className="text-red-500">*</span>}</label>
      <input type={type} value={form[name] || ''} onChange={e => update(name, type === 'number' ? Number(e.target.value) : e.target.value)} required={required} placeholder={placeholder} className="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-brand-500 outline-none" {...rest} />
    </div>
  );
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 sm:p-8">
        ...
        {step === 0 && (
          <div className="space-y-5">
            ...
            <Input label="Property Name" name="name" required placeholder="e.g. Mountain View Homestay" />
            ...
            <div>
              ...
              <textarea value={form.description || ''} onChange={e => update('description', e.target.value)} rows={5} placeholder="Detailed description of your property, rooms, surroundings, USPs..." className="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-brand-500 outline-none resize-none" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <Input label="Total Rooms" name="total_rooms" type="number" placeholder="e.g. 10" />
              <Input label="Year Established" name="year_established" type="number" placeholder="e.g. 2015" />
            </div>
            <h3 className="text-md font-semibold text-slate-800 pt-4">Contact Details (for this property)</h3>
            <div className="grid grid-cols-2 gap-4">
              <Input label="Contact Person Name" name="contact_name" required />
              <Input label="Contact Phone" name="contact_phone" required placeholder="+91..." />
              <Input label="Contact Email" name="contact_email" type="email" />
              <Input label="WhatsApp Number" name="contact_whatsapp" placeholder="+91..." />
            </div>
          </div>
        )}
        ...
        {step === 1 && (
          <div className="space-y-5">
            ...
            <Input label="Address Line 1" name="address_line1" required placeholder="Street, building name" />
            <Input label="Address Line 2" name="address_line2" placeholder="Locality, area" />
            <div className="grid grid-cols-3 gap-4">
              <Input label="City" name="city" required />
              <Input label="State" name="state" />
              <Input label="Pincode" name="pincode" placeholder="e.g. 176219" />
            </div>
            <Input label="Nearest Landmark" name="landmark" placeholder="e.g. Near Dalai Lama Temple" />
            <div className="grid grid-cols-2 gap-4">
              <Input label="Distance from Bus Stand" name="distance_from_bus_stand" placeholder="e.g. 2 km" />
              <Input label="Distance from Airport" name="distance_from_airport" placeholder="e.g. 18 km" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <Input label="Latitude" name="latitude" type="number" step="0.000001" placeholder="e.g. 32.2426" />
              <Input label="Longitude" name="longitude" type="number" step="0.000001" placeholder="e.g. 76.3213" />
            </div>
            <p className="text-xs text-slate-500">Tip: You can find coordinates by right-clicking your location on Google Maps.</p>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-5">
            ...
            <div className="flex items-center gap-4 pt-2">
              <Checkbox label="Tax Included in Price" checked={form.tax_included} onChange={() => update('tax_included', !form.tax_included)} />
              <Checkbox label="Accepts Online Payment" checked={form.accepts_online_payment} onChange={() => update('accepts_online_payment', !form.accepts_online_payment)} />
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">{PROPERTY_AMENITIES.map(a => <Checkbox key={a} label={a} checked={form.amenities.includes(a)} onChange={() => toggleArray('amenities', a)} />)}</div>
            </div>
            <div>
              <h3 className="text-sm font-semibold text-slate-700 mb-3">Room Amenities</h3>
              <div className="flex flex-wrap gap-2">{ROOM_AMENITIES.map(a => <Checkbox key={a} label={a} checked={form.room_amenities.includes(a)} onChange={() => toggleArray('room_amenities', a)} />)}</div>
            </div>
            <div>
              <h3 className="text-sm font-semibold text-slate-700 mb-3">Bathroom</h3>
              <div className="flex flex-wrap gap-2">{BATHROOM_AMENITIES.map(a => <Checkbox key={a} label={a} checked={form.bathroom_amenities.includes(a)} onChange={() => toggleArray('bathroom_amenities', a)} />)}</div>
            </div>
            <div>
              <h3 className="text-sm font-semibold text-slate-700 mb-3">Food & Dining</h3>
              <div className="flex flex-wrap gap-2">{FOOD_OPTIONS.map(a => <Checkbox key={a} label={a} checked={form.food_amenities.includes(a)} onChange={() => toggleArray('food_amenities', a)} />)}</div>
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="space-y-5">
            <h2 className="text-lg font-heading font-bold text-slate-900 mb-4">Property Photos</h2>
            <p className="text-sm text-slate-600 mb-4">Add photo URLs for your property. You can upload photos to Supabase Storage or use external URLs. Minimum 3 photos recommended.</p>
            <p className="text-xs text-amber-600 bg-amber-50 px-3 py-2 rounded-lg">Tip: For best results, use landscape photos (16:9 ratio) at least 1200px wide. Show rooms, exterior, views, bathroom, restaurant/café.</p>
            {(form.images || []).map((img: any, idx: number) => (
              <div key={idx} className="border border-slate-200 rounded-lg p-4 space-y-3">
                <div className="flex items-center justify-between"><span className="text-sm font-medium text-slate-700">Photo {idx + 1}</span><button onClick={() => update('images', form.images.filter((_: any, i: number) => i !== idx))} className="text-xs text-red-500">Remove</button></div>
                <input value={img.url} onChange={e => { const imgs = [...form.images]; imgs[idx] = { ...imgs[idx], url: e.target.value }; update('images', imgs); }} placeholder="https://... photo URL" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm outline-none" />
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <select value={img.category} onChange={e => { const imgs = [...form.images]; imgs[idx] = { ...imgs[idx], category: e.target.value }; update('images', imgs); }} className="px-3 py-2 border border-slate-300 rounded-lg text-sm outline-none">
                    <option value="exterior">Exterior</option><option value="room">Room</option><option value="bathroom">Bathroom</option><option value="restaurant">Restaurant</option><option value="view">View</option><option value="lobby">Lobby</option><option value="pool">Pool</option><option value="other">Other</option>
                  </select>
                  <Checkbox label="Primary Photo" checked={img.is_primary} onChange={() => { const imgs = form.images.map((im: any, i: number) => ({ ...im, is_primary: i === idx })); update('images', imgs); }} />
                </div>
              </div>
            ))}
            <button onClick={() => update('images', [...(form.images || []), { url: '', alt: '', category: 'exterior', is_primary: form.images.length === 0, sort_order: form.images.length }])} className="w-full border-2 border-dashed border-slate-300 rounded-xl py-3 text-sm font-medium text-slate-600 hover:border-brand-400 hover:text-brand-600">+ Add Photo</button>
          </div>
        )}
        ...
        {step === 5 && (
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-sm font-medium text-slate-700 mb-1">Alcohol Policy</label><input value={form.alcohol_policy || ''} onChange={e => update('alcohol_policy', e.target.value)} placeholder="e.g. Bar available / BYO / Not allowed" className="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm outline-none" /></div>
              <div><label className="block text-sm font-medium text-slate-700 mb-1">Extra Bed Charge (₹)</label><input type="number" value={form.extra_bed_charge || ''} onChange={e => update('extra_bed_charge', +e.target.value)} className="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm outline-none" /></div>
            </div>
            <Checkbox label="Couple Friendly (unmarried couples welcome)" checked={form.couple_friendly} onChange={() => update('couple_friendly', !form.couple_friendly)} />
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Child Policy</label>
              <input value={form.child_policy || ''} onChange={e => update('child_policy', e.target.value)} placeholder="e.g. Children under 5 stay free" className="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm outline-none" />
            </div>
            <div>
              <h3 className="text-sm font-semibold text-slate-700 mb-3">Accepted ID Types</h3>
              <div className="flex flex-wrap gap-2">{ID_TYPES.map(id => <Checkbox key={id} label={id} checked={form.id_required.includes(id)} onChange={() => toggleArray('id_required', id)} />)}</div>
            </div>
          </div>
        )}
