...
import { Car, Search, ArrowRight, MapPin, Clock, Users, MessageCircle, Plane, Route as RouteIcon, Map } from 'lucide-react';
import { Breadcrumb, FAQSection, SectionHeading } from '@/components/ui/Cards';
import BookingForm from '@/components/forms/BookingForm';
import { taxiRoutes, getTaxiRoutesByType } from '@/data/taxi';
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1">
              ...
              <select value={searchFrom} onChange={e => setSearchFrom(e.target.value)}
                ...
                <option>Dharamshala</option><option>Chandigarh</option><option>Delhi</option>
              </select>
            </div>
            <div className="flex-1">
              ...
              <select value={searchTo} onChange={e => setSearchTo(e.target.value)}
                ...
                <option>Pathankot</option><option>Manali</option><option>Delhi</option><option>Amritsar</option>
              </select>
            </div>
            <div className="flex-1">
            ...
            <div className="sm:self-end">
              <button className="flex items-center justify-center gap-2 bg-brand-600 text-white px-6 py-2.5 rounded-lg font-semibold hover:bg-brand-700 text-sm h-[42px] w-full sm:w-auto">
                  ...
                  <div key={r.id} className="bg-white border border-slate-200 rounded-xl p-4 sm:p-5 hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        ...
                        {r.includes && (r.includes as string[]).length > 0 && (
                          <div className="flex gap-1 mt-2">
                            {(r.includes as string[]).slice(0, 3).map(inc => (
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <p className="text-xs text-slate-500">{r.price_type === 'per_km' ? '/km' : 'one way'}</p>
