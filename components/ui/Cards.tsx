...
export function HotelCard({ hotel }: { hotel: Property }) {
  const img = hotel.images?.[0]?.url || 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=600&q=80';
  return (
    <Link href={`/hotels/${hotel.slug}`} className="group block">
      <div className="bg-white rounded-xl overflow-hidden border border-slate-200 hover:shadow-lg transition-all">
        <div className="relative aspect-[4/3] overflow-hidden">
          <Image src={img} alt={hotel.images?.[0]?.alt || hotel.name} fill className="object-cover group-hover:scale-105 transition-transform duration-300" sizes="(max-width:768px) 100vw, 33vw" />
        ...
        <div className="p-4">
          ...
          <p className="text-xs text-slate-500 capitalize mb-2">{hotel.type} · {hotel.review_count} reviews</p>
          <p className="text-sm text-slate-600 line-clamp-2 mb-3">{hotel.short_description}</p>
          <div className="flex items-center justify-between pt-2 border-t border-slate-100">
            <div><span className="text-lg font-bold text-slate-900">{formatPrice(hotel.price_min)}</span><span className="text-xs text-slate-500 ml-1">/ night</span></div>
  ...
  return (
    <Link href={`/destinations/${destination.slug}`} className="group block">
      <div className="relative aspect-[3/4] sm:aspect-[4/5] rounded-xl overflow-hidden">
        <Image src={destination.image} alt={destination.image_alt} fill className="object-cover group-hover:scale-105 transition-transform duration-500" sizes="(max-width:768px) 50vw, 20vw" />
    ...
    <Link href={`/treks/${trek.slug}`} className="group block">
      <div className="bg-white rounded-xl overflow-hidden border border-slate-200 hover:shadow-lg transition-all">
        <div className="relative aspect-[16/9]">
          <Image src={trek.images[0] || 'https://images.unsplash.com/photo-1551632811-561732d1e306?w=600&q=80'} alt={trek.name} fill className="object-cover group-hover:scale-105 transition-transform duration-300" sizes="(max-width:768px) 100vw, 33vw" />
        ...
        <div className="p-4">
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            <span className={cn('text-xs font-medium px-2 py-0.5 rounded-full', colors[trek.difficulty])}>{trek.difficulty}</span>
            <span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{trek.duration}</span>
            <span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{trek.max_altitude}</span>
          </div>
          <p className="text-sm text-slate-600 line-clamp-2 mb-3">{trek.short_description}</p>
          <div className="flex items-center justify-between pt-2 border-t border-slate-100">
...
export function BlogCard({ post }: { post: BlogPost }) {
  return (
    <Link href={`/blog/${post.slug}`} className="group block">
      <div className="bg-white rounded-xl overflow-hidden border border-slate-200 hover:shadow-lg transition-all">
        <div className="relative aspect-[16/9]">
          <Image src={post.image} alt={post.image_alt} fill className="object-cover group-hover:scale-105 transition-transform duration-300" sizes="(max-width:768px) 100vw, 33vw" />
        </div>
        <div className="p-4">
